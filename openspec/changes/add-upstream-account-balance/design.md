...
For a positive group multiplier, the local remaining quota is `max(identity_balance_usd, 0) / group_multiplier`. Every bound local account receives the full identity balance independently; the value is not divided by the number of local accounts. For each new authoritative balance observation, the sidecar writes `quota_limit = projected remaining` and resets `quota_used = 0`. The observation timestamp prevents repeated overview polling or startup reconciliation from resetting usage again before a genuinely newer balance snapshot arrives.

Sub2API interprets `quota_limit = 0` as unlimited. Therefore, an authoritative zero balance is the sole exception to the literal zero-used write: the sidecar atomically writes a tiny equal positive `quota_limit` and `quota_used` sentinel so `IsQuotaExceeded` is immediately true. A later positive balance replaces the sentinel with the projected limit and resets used quota to zero, automatically restoring eligibility. An exact zero group multiplier represents a non-consuming/unlimited upstream group and maps to `quota_limit = 0` with `quota_used = 0`.
...
- [Concurrent usage changes during quota projection] -> Treat each newer authoritative balance observation as a deliberate quota accounting reset; write `quota_limit` and `quota_used = 0` atomically through the existing Extra merge, and never reapply the same observation merely because local usage increased.
