...
这是一个独立旁路服务。它不导入或修改 Sub2API 后端代码。侧车不再主动调用账号测试或上游模型接口；账号表现来自 Sub2API 已有的真实请求性能聚合。除在线用户与实际成功率聚合视图外，管理页面继续通过现有 HTTP 接口汇总所有分组和账号。聚合视图可选使用专用只读 PostgreSQL 账号，未配置时不影响手动调度、上游管理、倍率保护、余额同步、告警或通知：
...
- `account_performance_minute`：按需增量读取最近一小时、按分组和账号隔离的真实上游尝试结果
- `account_performance_hourly`：按需读取最近 24 个完整 UTC 小时的成功率参考
...
- `GET /api/v1/admin/account-performance/health`：读取真实请求性能聚合采集健康状态
...
- API Key 账号显示今日用量、管理员配置的日/周/总额度、上游列表中已显式绑定 Key 的最终倍率，以及最近一小时真实请求成功率。成功率按 `success_count / (attempt_count - client_canceled_count)` 计算，并显示成功数、有效尝试数、低样本、无样本、过期或采集降级状态；悬停或聚焦可查看最近 24 个完整小时参考、客户端取消、账号切换与统计边界。最终倍率只在充值倍率和上游分组倍率均已同步时显示；未绑定、绑定失效、存在歧义或缺少配置时会显示原因，不会回退显示 Sub2API 探测倍率。
...
- 每个 API Key 账号保留“账号调度（全局）”手动开关。真实成功率只展示实际流量表现，不会自动启用、停用或恢复账号。
- 账号设置弹窗只管理账号级余额告警阈值；没有旧检测配置的 API Key 账号也可以直接设置。
...
- 分组可以设置默认余额告警阈值，账号设置可以配置账号级阈值；账号级阈值优先。例如分组阈值为 `50`、账号阈值为 `10` 时，该账号按 `10` 判断。
...
- Bark 请求失败只记录受限长度的错误，不阻塞手动调度、上游同步、余额更新或倍率保护，也不会在每次轮询时重复重试同一持续状态。可在设置弹窗中手动发送测试通知检查配置。
...
侧车启动时会进入被动模式：不启动旧的一秒检测循环，并把状态文件中所有存量检测规则持久化为禁用，清除下次执行时间、运行标记和连续成功/失败计数。旧策略、历史结果、检测消耗和加密直连材料仍保留在状态文件中，仅用于兼容回滚，不会被执行或显示。

- 如果账号仍带有 `ManagedSuspended=true`，说明旧检测状态机拥有这次停用；侧车会尝试通过现有调度接口恢复该账号，然后清除所有权。
- 管理员手动停止的账号没有 `ManagedSuspended` 所有权标记，被动模式不会自动打开。单个恢复失败只记录账号级错误，其他侧车功能继续启动，且该账号也不会继续主动检测。
- 旧检测配置的创建、更新、删除、立即执行、直连授权和撤销接口统一返回 HTTP `410 Gone` 与 `ACTIVE_PROBING_REMOVED`，处理器不会调用 Sub2API 账号测试、账号导出或任何上游模型接口。
- 最近一小时数据使用两分钟按需缓存。冷启动或空闲超过完整窗口时读取最多一小时；随后只重读新增尾部与两分钟重叠桶并替换重叠数据，吸收异步落库更新且不重复计数。管理员页面关闭后没有后台数据库轮询。
- 最近 24 小时参考仅使用当前 UTC 小时之前的 24 个完整小时，最多每五分钟刷新一次。查询失败时短期保留并明确标记最近一次成功结果，超过十分钟未更新后显示不可用，绝不回退扫描 `usage_logs`。
...
- 周期同步与实际成功率聚合读取、页面 10 秒概览刷新完全独立。一个身份失败只更新该身份的错误和尝试时间，其他身份及最近成功快照继续保留；失败身份的最后余额保留原观测时间并明确标记为旧数据，下一次成功同步后才替换并恢复为当前数据。

### 凭证安全与密钥轮换

`AUTO_SCHEDULER_CREDENTIAL_KEY` 是旁路敏感凭证专用的 32 字节密钥，用于保护上游登录会话和 Bark 通知凭证；状态文件里若仍有旧直连探测快照，也继续由同一密钥保护但不会被执行。密钥接受 base64 或 64 位 hex 编码。不要复用 Sub2API Admin API Key。生成 base64 密钥：
...
不配置密钥时服务仍会启动；分组账号总览、手动调度、实际成功率、余额告警阈值和不依赖秘密材料的功能保持工作，只有上游登录、凭证同步、自动匹配和 Bark 秘密配置返回可操作的禁用提示。所有上游 API 仍要求当前浏览器的有效管理员 JWT；自动精确匹配还会把该 JWT 转发给 Sub2API 现有账号导出接口，由其二次验证策略决定是否放行，绝不会回退到服务端 Admin API Key。
...
- 实际成功率只展示真实请求表现，不触发账号停用、恢复、重试或分组切换。账号全局调度只由管理员手动开关和 Sub2API 自身既有逻辑决定。
- 侧车不会调用账号测试接口或直接向账号上游发送模型请求，因此不会因健康展示消耗上游额度，也不会形成固定频率的模型请求流量。
- 旧检测配置、最近 50 次历史、检测消耗和直连授权密文保留为休眠兼容数据；启动时规则会被禁用，所有写入/执行入口均返回 410。
- 分组默认保护倍率、账号级保护倍率、逻辑分组成员关系、休眠检测兼容数据、余额告警阈值和通知边沿状态都只保存在旁路状态文件中，文件继续使用 `0600` 权限原子写入。
- 不修改 Sub2API 数据库结构、正式计费倍率、账号额度计费、调度算法或生产源码。对 Sub2API 的写操作仍仅通过现有 HTTP 接口完成管理员明确请求的调度开关、账号余额同步和实际分组绑定变更。

## 部署

1. 在现有 Sub2API 管理后台的系统设置中生成 Admin API Key。
2. 将 `.env.example` 复制为 `.env`，填写 Admin API Key、主服务地址和外部访问 URL。若启用上游连接或 Bark 加密推送，再生成并填写独立的 `AUTO_SCHEDULER_CREDENTIAL_KEY`。
3. 启动独立服务：
...
在线人数和实际成功率功能共用一个独立 PostgreSQL 只读账号，不复用 Sub2API 的写账号。下面的 SQL 由数据库管理员执行一次；密码应随机生成并仅写入侧车服务器的 `.env`，不要提交到仓库：
...
2026 年 9 月 3 日这次被动模式更新在 HC2 发布前，应先备份侧车状态文件：

```bash
docker cp account-auto-scheduler:/data/state.json ./state.pre-passive-20260903.json
```

本次不修改状态文件版本或数据库结构。新侧车首次启动会保留历史与上游数据，但会把所有旧检测策略写为禁用，并尝试恢复仍由旧检测状态机拥有的停用账号。发布前必须先为 `sidecar_reader` 增加两张账号性能聚合表的 `SELECT` 权限；缺少权限不会影响其他功能，但实际成功率会显示不可用。发布只允许执行：

```bash
docker compose -f compose.hc2.yml up -d --build --no-deps account-auto-scheduler
```

发布前后分别记录全部容器的名称、ID 和 restart count，并确认除 `account-auto-scheduler` 外没有容器被重建或重启。若需要回滚，只替换该旁路容器；当前状态与旧镜像兼容，但旧策略已被持久化禁用，不会自动恢复主动检测。只有管理员明确决定恢复旧主动探测时，才应在回滚后重新启用规则；直接恢复部署前状态备份可能同时恢复当时启用的主动规则。发布和回滚都不要重建 Sub2API、数据库、Redis、Nginx 或 Nginx UI。
