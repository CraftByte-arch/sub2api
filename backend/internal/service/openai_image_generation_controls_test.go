...
func TestOpenAIGatewayServiceForward_RejectsDisabledImageGenerationIntents(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			body: []byte(`{"model":"gpt-5.4","input":"draw","tools":[{"type":"namespace","name":"image_gen","tools":[{"type":"function","name":"imagegen"}]}]}`),
		},
		{
...
func TestOpenAIGatewayServiceForward_DisabledGroupStripsPassiveImageToolAndForwardsText(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	c, recorder := newOpenAIImageGenerationControlTestContext(false, "unit-test-agent/1.0")
	account := newOpenAIImageGenerationControlTestAccount()
	body := []byte(`{"model":"gpt-5.4","input":"write code","stream":false,"tools":[{"type":"image_generation"}]}`)
	...
	require.Equal(t, "write code", gjson.GetBytes(upstream.lastBody, "input").String())
}
...
func TestOpenAIGatewayServiceForward_CodexBridgeCanonicalizesImageGenNamespace(t *testing.T) {
	gin.SetMode(gin.TestMode)
