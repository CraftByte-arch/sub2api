...
func writeOpenAIPassthroughErrorEnvelope(c *gin.Context, downstreamStatus int, upstreamHeaders http.Header, message string) {
	...
	bridgeHandled, bridgeErr := writeOpenAICompactSSEBridge(c, downstreamStatus, body)
	if bridgeErr == nil && bridgeHandled {
		return
	}
...
func (s *OpenAIGatewayService) recordOpenAIStreamUpstreamError(
	...
	if c != nil && isFirstTokenTimeoutCause(firstTokenContextFromGin(c), nil) {
		return message
	}
	statusCode := openAIStreamFailureStatus(payload, message)
	detail := ""
