...
func TestExecuteUsageCardFulfillmentRecoversStaleLease(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
	ensurePaymentAuditOrderActionUniqueIndex(t, ctx, client)
	staleAt := time.Now().Add(-paymentFulfillmentLeaseDuration - time.Minute)
	...
	svc := &PaymentService{
		entClient:        client,
		usageCardService: NewUsageCardService(usageRepo, nil),
	}

	require.NoError(t, svc.ExecuteUsageCardFulfillment(ctx, order.ID))
