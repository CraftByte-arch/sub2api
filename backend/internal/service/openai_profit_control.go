...
//     openai/grok 分组。
//   - U（上游成本倍率）对 API Key 优先取可选的 final-cost 调度信号，否则取
//     accounts.rate_multiplier。两者始终独立，计费仍只读取 rate_multiplier。
...
func openAIProfitControlVetoReason(ctx context.Context, account *Account) (bool, string) {
	...
	cost, valid := resolveProfitSchedulingCostSignal(account)
	if !valid {
	...
	if profitControlOverThreshold(cost.multiplier, gate.threshold) {
