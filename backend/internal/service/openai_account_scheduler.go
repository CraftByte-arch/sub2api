...
// SelectAccountWithSchedulerForImageProtocol preserves the image-studio
// scheduling entry point. The main scheduler applies its standard account
// ranking while the selected account still enforces its protocol capability.
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForImageProtocol(
	ctx context.Context,
	groupID *int64,
	previousResponseID string,
	sessionHash string,
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	selection, decision, err := s.selectAccountWithSchedulerOnce(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, imageProtocolPreference, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
	...
	return s.selectAccountWithSchedulerOnce(withOpenAIProxyStreamQuarantineBypass(ctx), groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, imageProtocolPreference, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
}
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	if scheduler == nil {
		...
		if requiredTransport == OpenAIUpstreamTransportAny || requiredTransport == OpenAIUpstreamTransportHTTPSSE {
			...
			for {
				selection, err := s.selectAccountWithLoadAwareness(ctx, groupID, platform, legacySessionHash, requestedModel, effectiveExcludedIDs, requireCompact, requiredCapability, imageProtocolPreference, useUpstreamTokenCost)
				if err != nil {
					return nil, decision, err
				}
				if selection == nil || selection.Account == nil {
...
// ReportOpenAIAccountFirstTokenTimeout records a timeout without changing the
// persisted account state. Model-specific accounting is unavailable here, so
// use the scheduler's neutral model bucket.
