...
func (s *SettingService) InitializeDefaultSettings(ctx context.Context) error {
	...
	defaults := map[string]string{
		...
		SettingKeyLegacySubscriptionPurchaseEnabled:         "true",
		SettingKeyLegacySubscriptionVisible:                 "true",
		SettingKeyUsageCardEnabled:                          "false",
		SettingKeyUsageCardPaymentEnabled:                   "false",
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	result := &SystemSettings{
		...
		OpenAILongContextBillingMultiplier: parsePositiveFloatSetting(
			settings[SettingKeyOpenAILongContextBillingMultiplier],
			defaultOpenAILongContextBillingMultiplier,
		),
		OpenAILongContextOutputMultiplier: parsePositiveFloatSetting(
			settings[SettingKeyOpenAILongContextOutputMultiplier],
			defaultOpenAILongContextOutputMultiplier,
		),
...
func parseInt64ListSetting(raw string) []int64 {
	...
	return normalizeImageStudioAvailableGroupIDs(values)
}

func marshalInt64ListSetting(ids []int64) string {
	data, err := json.Marshal(normalizeImageStudioAvailableGroupIDs(ids))
	if err != nil {
		return "[]"
	}
	return string(data)
