...
type ModelPricing struct {
	...
	LongContextPolicyExplicit          bool     // 价格目录是否显式配置 long_context_*（显式 0 也必须保留）
	...
	if s.pricingService != nil {
		...
		if litellmPricing != nil {
			...
			return s.applyModelSpecificPricingPolicy(model, &ModelPricing{
				...
				LongContextPolicyExplicit:     litellmPricing.LongContextPolicyExplicit,
...
// 档按业务倍率改写（本地/远程目录的 priority 价可能沿用官方旧口径）。目录中的
// 显式长上下文策略优先保留；未显式配置时使用后台运行时策略。默认强制 DeepSeek
// 官方价——该路径仅被默认价卡（GetModelPricing 内部）调用；分组/渠道自定义
// 定价路径用带参数的 applyModelSpecificPricingPolicyEx 关闭强制，保留运营者配置。
...
func (s *BillingService) applyModelSpecificPricingPolicyEx(model string, pricing *ModelPricing, forceDeepSeekRates bool) *ModelPricing {
	...
	hasCustomLongContextPolicy := pricing.LongContextInputThreshold > 0 &&
		...
		(pricing.LongContextInputThreshold != defaultOpenAILongContextBillingThreshold ||
			pricing.LongContextInputMultiplier != defaultOpenAILongContextBillingMultiplier ||
			pricing.LongContextOutputMultiplier != defaultOpenAILongContextOutputMultiplier)
	...
	needsRuntimeLongContextPolicy := s != nil && s.settingService != nil &&
		usesLegacyLongContextPricing && !pricing.LongContextPolicyExplicit && !hasCustomLongContextPolicy
	if !needsRuntimeLongContextPolicy && !needsCacheCreationPolicy && fastRatio <= 0 && !needsMaxReasoningEffortMultiplier {
		return pricing
	}
	cloned := *pricing
	...
	if needsRuntimeLongContextPolicy {
		if !runtime.Enabled {
