...
func (s *OpenAIGatewayService) ForwardImagesOAuthEdit(
	...
	// Stored multi-image edits are represented through the Responses image-generation tool.
	// Do not let the direct Images API routing added for ordinary OAuth requests bypass it.
	return s.forwardOpenAIImagesOAuth(withOpenAIImagesForceResponses(ctx), c, account, &request, channelMappedModel)
}
