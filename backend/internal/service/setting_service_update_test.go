...
func TestSettingService_UpdateSettings_ImageStudioAsyncSettings(t *testing.T) {
	repo := &settingUpdateRepoStub{}
	svc := NewSettingService(repo, &config.Config{})

	err := svc.UpdateSettings(context.Background(), &SystemSettings{
		ImageStudioAsyncConcurrency: 5,
		ImageStudioRetentionValue:   48,
		ImageStudioRetentionUnit:    ImageStudioRetentionUnitHour,
	})
	require.NoError(t, err)
	require.Equal(t, "5", repo.updates[SettingKeyImageStudioAsyncConcurrency])
	require.Equal(t, "48", repo.updates[SettingKeyImageStudioRetentionValue])
