...
type UserHandler struct {
	...
	totpService           *service.TotpService                // 特权角色授权的 step-up 门控
...
type CreateUserRequest struct {
	...
	Role                 string   `json:"role" binding:"omitempty,oneof=admin user invitation_expert"`
...
func (h *UserHandler) Create(c *gin.Context) {
	...
	// 创建特权角色账号会授予额外管理或个人信息访问权限，需最近完成 step-up 2FA 验证。
	if service.IsPrivilegedUserRole(req.Role) {
...
func (h *UserHandler) Update(c *gin.Context) {
	...
	// 防锁死保护：管理员不能把自己降级为任意非管理员角色（会失去后台访问权）。
	...
	// 授予特权角色（管理员或邀请达人）需最近完成 step-up 2FA 验证。
	// 目标已持有相同角色时（前端编辑表单总是携带 role）不触发，避免日常编辑被打断。
	if service.IsPrivilegedUserRole(req.Role) {
