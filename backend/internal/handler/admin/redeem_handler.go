...
type GenerateRedeemCodesRequest struct {
	...
	Type            string     `json:"type" binding:"required,oneof=balance concurrency subscription invitation usage_card"`
	Value           float64    `json:"value"`
	GroupID         *int64     `json:"group_id"`           // 订阅类型必填
	UsageCardPlanID *int64     `json:"usage_card_plan_id"` // 余额卡类型必填
...
type CreateAndRedeemCodeRequest struct {
	...
	Type            string     `json:"type" binding:"omitempty,oneof=balance concurrency subscription invitation usage_card"` // 不传时默认 balance（向后兼容）
	Value           float64    `json:"value"`
	UserID          int64      `json:"user_id" binding:"required,gt=0"`
	GroupID         *int64     `json:"group_id"`           // subscription 类型必填
	UsageCardPlanID *int64     `json:"usage_card_plan_id"` // usage_card 类型必填
	ValidityDays    int        `json:"validity_days"`      // subscription 类型有效天数
...
func (h *RedeemHandler) Generate(c *gin.Context) {
	...
	executeAdminIdempotentJSON(c, "admin.redeem_codes.generate", req, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		codes, execErr := h.adminService.GenerateRedeemCodes(ctx, &service.GenerateRedeemCodesInput{
			...
			Value:           req.Value,
			GroupID:         req.GroupID,
			UsageCardPlanID: req.UsageCardPlanID,
...
func (h *RedeemHandler) CreateAndRedeem(c *gin.Context) {
	...
	if req.Type == "usage_card" {
		if req.GroupID != nil {
			response.BadRequest(c, "group_id must be empty for usage card type")
			return
		}
		if req.UsageCardPlanID == nil || *req.UsageCardPlanID <= 0 {
			response.BadRequest(c, "usage_card_plan_id is required for usage card type")
			return
		}
		if req.ValidityDays != 0 {
			response.BadRequest(c, "validity_days must be empty for usage card type")
			return
		}
	}
